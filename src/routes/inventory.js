...
const validateZeroStockBody = (body) => {
  const errors = {};
  const { fullItemCode, baseItemCode } = normalizeItemCode(body && body.itemCode);
  const branchCode =
    body && typeof body.branchCode === "string" ? body.branchCode.trim() : "";
...
router.post("/zero-stock", authJwt(), async (req, res, next) => {
  try {
    const { itemCode, baseItemCode, branchCode, errors } = validateZeroStockBody(
      req.body
    );
    if (Object.keys(errors).length > 0) {
      return res.status(400).json({
    ...
    const stockResult = await pool
      ...
      .query(
        ...
        WHERE (@ItemCode = '' OR ItemCode = @ItemCode OR LEFT(ItemCode, 6) = @BaseItemCode)
        GROUP BY
          Category,
          OldCode,
          ItemCode,
